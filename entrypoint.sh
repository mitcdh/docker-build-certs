...
CERTIFICATE_CN="${CERTIFICATE_CN:-certificate}"
CERTIFICATE_SAN="${CERTIFICATE_SAN:-certificate}"
...
generate_certs() {
    ...
    cat > "$CERT_DIR/certificate_csr.conf" <<-EOF
    [req]
...
EOF
    ...
    do
        echo "DNS.$index = $san" >> "$CERT_DIR/certificate_csr.conf"
        index=$((index+1))
    done
    IFS=$OLDIFS

    # Generate a CSR
    openssl req -new -key "$KEY_FILE" -out "$CERT_DIR/certificate.csr" -config "$CERT_DIR/certificate_csr.conf"

    # Sign the CSR with the CA certificate to generate the certificate
    openssl x509 -req -in "$CERT_DIR/certificate.csr" -CA "$CERT_DIR/ca.crt" -CAkey "$CERT_DIR/ca.key" -CAcreateserial \
        -out "$CERT_FILE" -days "$DAYS_VALID" -sha256 -extfile "$CERT_DIR/certificate_csr.conf" -extensions v3_req
}
